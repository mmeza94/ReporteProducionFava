...
namespace Tenaris.Fava.Production.Reporting.ViewModel.Support
{
    public class ReportConfirmationSupport
    {
        ...
        public static bool ReportConfirmation(int tbGoodCountL2, int tbScrapCountL2, int tbReworkedCountL2, int tbLoadedCountL2, string extremo, int epProductionReport
            ...
            , int lbITLoadHelper, int tbTotalLoaded, string SelectedSendType, int tbPreviousLoaded, string User, InteractionRequest<Notification> ShowQuestionRequest,
            InteractionRequest<Notification> showMessageRequest, InteractionRequest<Notification> showErrorWindowRequest)
        {
            user = User;
            ...
            int tbTotalLoaded, InteractionRequest<Notification> showMessageRequest)
        {
            if (response.StartsWith("Reporte Enviado Correctamente"))
            {
                ...
                var isAvailable = new ITServiceAdapter().IsGroupItemAvailableForNextOperation(currentProductionReportDto.IdUDT,
        ...
        //public static ReportProductionDto PrepareDtoForProductionReport(ReportProductionDto currentProductionReportDto, int lbITLoadHelper, int tbPreviousLoaded,
        //    int tbLoadedCountL2, int tbGoodCountL2, int tbScrapCountL2, int tbReworkedCountL2, string user, string SelectedSendType)
        //{
        //    if (Configurations.Instance.Machine.ToUpper().Equals("CND"))
        //    {
        //        // PRUEBAS BGR

        //        // EL DECOMPILADO INCORRECTO
        //        //currentProductionReportDto.CantidadTotal = tbLoadedCountL2;

        //        //EL QUE ESTA EN PRODUCTIVO AHORITA
        //        if (SelectedSendType.Equals("Parcial"))
        //        {
        //            currentProductionReportDto.CantidadTotal = tbGoodCountL2 + tbScrapCountL2;
