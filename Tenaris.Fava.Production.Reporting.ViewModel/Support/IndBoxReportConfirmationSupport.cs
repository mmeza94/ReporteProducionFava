...
namespace Tenaris.Fava.Production.Reporting.ViewModel.Support
{
    public class IndBoxReportConfirmationSupport
    {
            ...
            ObservableCollection<ProductionBox> dgBoxes, string user, InteractionRequest<Notification> questionRequest, InteractionRequest<Notification> messageRequest,InteractionRequest<Notification> errorMessageRequest)
        {
            ...
            try
            {
                //PRUEBADWF CAJAS HARDCODE BORRAR
                //ProductionBox cajon = new ProductionBox()
                //{
                //    Id = "123",
                //    ParentOrderNumber = 1,
                //    OrderNumber = 123,
                ...
                if (selectedBox != null)
                {
                    ...
                    if (goodPieces <= selectedBox.MissingPieces)
                    {
                        ...
                        if (showQuestion2.Result)
                        {
                            ...
                            int defaultSequence = currentProductionReport.Secuencia;

                            //PRUEBADWF OBTENER NUEVA SECUENCIA REAL CAJA PARA ENVIO A TPS (BARRAS DE PESO)
                            DataTable availableItems = iTServiceAdapter.GetAvailableStock(currentProductionReport.Orden.ToString(), (currentProductionReport.Secuencia));
                            ...
                            currentProductionReport.Secuencia = Convert.ToInt32(items.FirstOrDefault()?.secuenciaRealx) == 0 ? currentProductionReport.Secuencia : Convert.ToInt32(items.FirstOrDefault()?.secuenciaRealx);
