    ...
    public class ProductionReport
    {
        ...
        public static ObservableCollection<GeneralPiece> GetSendStatusForGeneralPieces(IList<GeneralPiece> generalPieces)
        {
            ...
            if (Configurations.Instance.Machine != "Pintado")
            {
                foreach (GeneralPiece generalpiece in generalPieces)
                {
                    ...
                    {
                        var reportProductionHistory = reportProductionHistoryFacade.GetReportProductionHistoryByIdHistory(generalpiece.Extremo, generalpiece.OrderNumber, generalpiece.HeatNumber, generalpiece.GroupItemNumber);
                        generalpiece.Sended = (reportProductionHistory != null) ? Enumerations.AxlrBit.Si : Enumerations.AxlrBit.No;
                        generalpiece.SendedString = (reportProductionHistory != null) ? "Si" : "No";
                    }
        ...
        public string ReportProductionForPainting(PaintingReport reportProductionDto, Enumerations.ProductionReportSendStatus sendStatus, bool loadMaterial, IList rejectionReportDetails)
        {
            ...
            try
            {
                ...
                int defaultSequence = Convert.ToInt32(Configurations.Instance.Secuencia);
                respuesta = new ITServiceAdapter().TPSReportProduction(reportProductionDto.IdUser,reportProductionDtoToSend, sendStatus, rejectionReportDetails, 3, defaultSequence);
            }
            catch (Exception ex)
            {
                respuesta = ex.Message.ToString();
            ...
            try
            {
                ...
                if ( Configurations.Instance.MaquinaInicialZonaFiltre == "0" || Configurations.Instance.MachineFiltre == "Horno de Revenido" || Configurations.Instance.MachineFiltre == "Horno de Normalizado" || Configurations.Instance.Machine.Contains("Roscadora") && Configurations.Instance.Extremo == "Extremo 2")
                {
                    ...
                    else
                    {
                        ...
                        int FoundLoadedCount = instancia.GetLastMachineGoodPieces(somePieces[0].OrderNumber, somePieces[0].HeatNumber, somePieces[0].GroupItemNumber, somePieces[0].Description, somePieces[0].Extremo);
